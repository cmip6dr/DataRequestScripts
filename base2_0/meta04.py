## pydoc imported to make the example scripts run smoothly
import pydoc
...
class Example01(object):
    """A class for instances with attributes 'this' and 'that'
       Using slots for efficiency as there may be many instance of this class
       Attribute values will be stored in instances of a class associated with each attribute.
       For an attribute 'x', 'instance.x' will return the value, ad 'instance._x' will return
       the object which may contain additional information and further attributes.
    """
    attribute_classes = dict( this=ClassThis, that=ClassThat )
    slots = ('this','that', '_this', '_that')
    required = {'that'}
...
strhelp = pydoc.render_doc(x.attribute_classes['this'], "Help on attribute of x:  %s")
print (strhelp )
...
##
import collections
print ('============ Testing a Metaclass approach to class creation ===========')
